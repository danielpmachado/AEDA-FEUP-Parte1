...
int Company::cancelReservation() {
	...
	if (num_days >= 30) {
		...
		cout << TAB << "A totalidade do valor (" << price << ") ser-lhe-á devolvida." << endl;

	}
	else if (num_days >= 15){
		cout << TAB << "A sua reserva foi cancelada com sucesso." << endl;
		cout << TAB << "Apenas 50% do valor (" << price / 2 << ") ser-lhe-á devolvida." << endl;

	}
	else {
		cout << TAB << "A sua reserva foi cancelada com sucesso." << endl;
		cout << TAB << "Por a reserva ter sido cancelada com pouca antecedência não será reembolsadao." << endl;
	}
